...
def get_test_dataset_evaluation() -> Tuple[Optional[pd.DataFrame], Optional[float]]:
    """
    ...
    Returns (None, None) if live evaluation fails and no stored metrics exist —
    callers must handle None and show "metrics unavailable" rather than fabricating numbers.
    """
    try:
        ...
        logger.warning(f"Live evaluation failed: {e}. Attempting to load stored model_metrics.json...")
        stored = load_model_metrics()
        if stored and "hybrid" in stored:
            s = stored.get("sarimax", {})
            l = stored.get("lstm_residual", {})
            h = stored.get("hybrid", {})
            # Only use stored values that are actually present — no hardcoded placeholders
            def _row(name, d):
                return {
                    "Model": name,
            ...
            metrics_df = pd.DataFrame([
                _row("SARIMAX", s),
                _row("LSTM Residual Model", l),
                _row("Hybrid SARIMAX-LSTM", h),
            ])
            acc = h.get("accuracy")
            return metrics_df, round(acc, 2) if acc is not None else None

        # No live data and no stored metrics — return None so the UI can show a proper warning.
        logger.warning("No stored model_metrics.json found. Returning None — UI should display metrics-unavailable notice.")
        return None, None
